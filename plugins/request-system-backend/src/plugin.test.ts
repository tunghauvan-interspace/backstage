...
import { requestSystemPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [requestSystemPlugin],
    });

    await request(server).get('/api/request-system/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/request-system/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/request-system/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/request-system/todos/${createdTodoItem.id}`)
