...
import { approvalPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [approvalPlugin],
    });

    await request(server).get('/api/approval/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/approval/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/approval/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/approval/todos/${createdTodoItem.id}`)
